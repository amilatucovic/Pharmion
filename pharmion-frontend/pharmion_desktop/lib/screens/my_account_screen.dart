...
class _MyAccountScreenState extends State<MyAccountScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      final data = await ApiService.get('Pharmacist/me') as Map<String, dynamic>;
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null) return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(_error!, style: const TextStyle(color: AppColors.kTextMid, fontSize: 14)),
        const SizedBox(height: 16),
        ElevatedButton(onPressed: _loadProfile,
            style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.kTeal, foregroundColor: Colors.white),
              ...
              Expanded(child: Column(
                ...
                Text('$_firstName $_lastName',
                    style: const TextStyle(fontSize: 22,
                        fontWeight: FontWeight.w800, color: AppColors.kTextDark)),
                const SizedBox(height: 4),
                Text('@$_username',
                    style: const TextStyle(fontSize: 13, color: AppColors.kTextMid)),
                const SizedBox(height: 8),
                Row(children: [
                  _Badge(label: _role, color: AppColors.kTeal,
                      bg: AppColors.kTealLight, icon: Icons.badge_outlined),
                  if (_isAdmin) ...[
                    const SizedBox(width: 8),
                    _Badge(label: 'Administrator',
                  ...
                  _Badge(
                      ...
                      bg: _isActive ? const Color(0xFFD1FAE5) : const Color(0xFFF1F5F9),
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              children: [
                ...
                _InfoRow(label: 'Gender',
                    value: _gender == '0' || _gender.toLowerCase() == 'male'
                        ? 'Male' : _gender == '1' || _gender.toLowerCase() == 'female'
                        ? 'Female' : '—'),
              ],
            )),
            ...
            Expanded(child: _InfoCard(
              ...
              children: [
                ...
                _InfoRow(label: 'Administrator', value: _isAdmin ? 'Yes' : 'No',
                    valueColor: _isAdmin
                        ? const Color(0xFF059669) : AppColors.kTextMid),
                _InfoRow(label: 'License No.',
                    value: _licenseNumber.isNotEmpty ? _licenseNumber : '—'),
                _InfoRow(label: 'Pharmacy',
              ...
              children: [
                _InfoRow(label: 'Status',
                    ...
                    valueColor: _isActive
                        ? const Color(0xFF059669) : const Color(0xFF64748B)),
                _InfoRow(label: 'Member Since', value: _fmtDate(_createdAt)),
                _InfoRow(label: 'Last Login', value: _fmtDateTime(_lastLoginAt)),
                const SizedBox(height: 8),
...
class _EditProfileDialogState extends State<_EditProfileDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final current = await ApiService.get('Pharmacist/${widget.userId}')
          ...
          child: Column(mainAxisSize: MainAxisSize.min,
            ...
            Row(children: [
              Container(width: 36, height: 36,
                decoration: BoxDecoration(color: AppColors.kTealLight,
                    borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.edit_outlined, size: 18, color: AppColors.kTeal)),
              const SizedBox(width: 12),
              const Text('Edit Profile', style: TextStyle(fontSize: 16,
                  fontWeight: FontWeight.bold, color: AppColors.kTextDark)),
              const Spacer(),
              IconButton(onPressed: () => Navigator.pop(context),
                  icon: const Icon(Icons.close, size: 18, color: AppColors.kTextMid),
            ...
            // First + Last name
            Row(children: [
              Expanded(child: _FieldCol(label: 'First Name *',
                  child: TextField(controller: _firstNameCtrl,
                      style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
                      decoration: _deco('First Name')))),
              const SizedBox(width: 12),
              Expanded(child: _FieldCol(label: 'Last Name *',
                  child: TextField(controller: _lastNameCtrl,
                      style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
            ...
            // License
            _FieldCol(label: 'License Number *',
              child: TextField(controller: _licenseCtrl,
                  style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(foregroundColor: AppColors.kTextMid,
              ...
              ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.kTeal,
                    foregroundColor: Colors.white, elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10)),
              ),
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          Text(label, style: const TextStyle(fontSize: 12,
              fontWeight: FontWeight.w600, color: AppColors.kTextMid)),
          const SizedBox(height: 6),
          child,
        ]);
}
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Text(title, style: const TextStyle(fontSize: 13,
            fontWeight: FontWeight.w700, color: AppColors.kTextDark)),
      ]),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      SizedBox(width: 110, child: Text(label, style: const TextStyle(
          fontSize: 12, color: AppColors.kTextMid, fontWeight: FontWeight.w500))),
...
class _ChangePasswordDialogState extends State<_ChangePasswordDialog> {
  ...
  Future<void> _save() async {
    ...
    if (oldPass.isEmpty) { setState(() => _error = 'Current password is required.'); return; }
    if (newPass.length < 8) { setState(() => _error = 'New password must be at least 8 characters.'); return; }
    if (newPass != confirm) { setState(() => _error = 'Passwords do not match.'); return; }
    if (oldPass == newPass) { setState(() => _error = 'New password must be different from the current one.'); return; }
  ...
  Widget build(BuildContext context) {
    return Dialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: SizedBox(width: 420, child: Padding(
        padding: const EdgeInsets.all(24),
        child: Column(mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(width: 36, height: 36,
                decoration: BoxDecoration(color: AppColors.kTealLight,
                    borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.lock_outline, size: 18, color: AppColors.kTeal)),
            const SizedBox(width: 12),
          ...
          TextField(controller: _oldCtrl, obscureText: !_showOld,
              style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
              decoration: _deco('Current Password', _showOld,
                  () => setState(() => _showOld = !_showOld))),
          const SizedBox(height: 12),
          TextField(controller: _newCtrl, obscureText: !_showNew,
              style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
              decoration: _deco('New Password', _showNew,
                  () => setState(() => _showNew = !_showNew))),
          const SizedBox(height: 12),
          TextField(controller: _confirmCtrl, obscureText: !_showConfirm,
              style: const TextStyle(fontSize: 13, color: AppColors.kTextDark),
              decoration: _deco('Confirm New Password', _showConfirm,
          ...
          Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Update Password', style: TextStyle(fontSize: 13)),
              style: ElevatedButton.styleFrom(backgroundColor: AppColors.kTeal,
                  foregroundColor: Colors.white, elevation: 0,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10)),
